# Legend placement findings: GenomeSpy, Vega, and Vega-Lite
...
- GenomeSpy's implementation before the proposed collection work, principally
...
## Independent composition case

The Core example `examples/core/legends/threshold-gradient.json` provides a
representative independent-resolution comparison. It contains two horizontally
concatenated views with independent color scales, a right legend in the first
view, and a bottom legend in the second.

Vega-Lite leaves these independent legends in their child groups. The right
legend therefore occupies space between the plots, and the bottom legend is
sized relative to the second plot. GenomeSpy's existing resolution-owner layout
produces the same ownership pattern: an independent legend remains with its
`GridChild`, while a shared legend is laid out by the `GridView` that owns its
resolution.

Moving both legends unconditionally to the visualization root changes that
result. The right legend moves outside the complete composition, and the bottom
legend uses the complete root width. This demonstrates that root collection is
not Vega-Lite's default behavior and must remain distinct from normal
shared-versus-independent resolution.
