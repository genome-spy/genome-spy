...
4. If the development server is not running, start `npm start` from the
   repository root and keep the session available while testing.
5. The root page lists example and private specs; the first example is usually
   the quickest smoke test.
6. Reproduce the smallest relevant flow and inspect visible output, console
   errors, and network behavior as appropriate.
7. After a fix, repeat the reproduction and add durable automated coverage when
