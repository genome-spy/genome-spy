...
float sdArrowHeads(
    ...
    float repeatX = terminalTipX - offset - p.x;
...
float stemEndForHead(float edge, float headInset, float overlap) {
...
float sdArrow(vec2 p, vec2 halfSize) {
    ...
    float terminalHeadLength = headLength;
    float headCount = 1.0;
    float maxHeadLength = arrowLength;
    bool squeezeHead = uHeadPlacement == HEAD_PLACEMENT_INSIDE;
    bool shortForHeads = squeezeHead && terminalHeadLength > maxHeadLength;
    if (shortForHeads && uShortArrow == SHORT_ARROW_HIDE) {
        return 1.0;
    }
    if (squeezeHead) {
        terminalHeadLength = min(terminalHeadLength, maxHeadLength);
    }
    ...
    float stemLength = max(arrowLength - terminalHeadLength * headCount, 0.0);
    if (uHeadShape == HEAD_SHAPE_TRIANGLE) {
        if (squeezeHead) {
            float notchScale = terminalHeadLength > 0.0
                ? clamp(stemLength / terminalHeadLength, 0.0, 1.0)
                : 0.0;
    ...
    float stemLeft = -b.x;
    float stemRight = b.x;
    if (terminalHeadLength > 0.0) {
        if (uHeadShape == HEAD_SHAPE_TRIANGLE) {
            float headBaseX = b.x - terminalHeadLength;
            stemRight = mix(headBaseX, b.x, headNotchDepth);
        } else {
            float overlap = stemHeadOverlap(terminalHeadLength);
            stemRight = stemEndForHead(
                b.x,
                terminalHeadLength,
                overlap
            );
    ...
    if (terminalHeadLength > 0.0 && headHalfWidth > 0.0) {
