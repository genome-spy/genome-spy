...
/**
 * Creates the effective visible bounds for a mark. The root viewport
 ...
 * @returns {RenderBounds}
 */
export function createVisibleBounds(width, height, clip) {
    return {
 ...
 * @returns {RenderBounds}
 */
export function createAnchorCullBounds(coords, clip, cull) {
 ...
 * @param {number} y
 */
export function isOutsideBounds(bounds, x, y) {
 ...
 *
 * @param {RenderBounds} visibleBounds
 * @param {number} x1
 * @param {number} y1
 * @param {number} x2
 * @param {number} y2
 ...
 */
export function intersectsBounds(visibleBounds, x1, y1, x2, y2, padding = 0) {
    return !(
