...
describe("Domain handling", () => {
     ...
     */
    const createLinkedDomainSpec = (domain, linkZoom = true) => ({
    ...
    test("selection-linked domains react to pushed outer interval params", async () => {
        const view = await createAndInitialize(
            createLinkedDomainSpec({ param: "brush", encoding: "x" }),
            ConcatView
        );
    ...
    test("selection-linked domains skip zoomTo animation during continuous updates", async () => {
        const view = await createAndInitialize(
            createLinkedDomainSpec({ param: "brush", encoding: "x" }, false),
            ConcatView
        );
    ...
    test("two-way linked domains write domain updates back to interval params", async () => {
        const view = await createAndInitialize(
