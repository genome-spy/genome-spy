...
export function renderRuleCanvas(baseMark, options) {
    ...
    context.lineCap = resolveSvgProperty(mark, mark.properties.strokeCap);
    context.setLineDash(
        resolveSvgProperty(mark, mark.properties.strokeDash) ?? []
    );
    context.lineDashOffset = resolveSvgProperty(
        mark,
