...
Band/index padding expressions use the production mapping manager. They share
range binding, dependency collection and application;
explicit inner/outer padding overrides general padding regardless of property order.
Step-sized views observe mapping changes to invalidate their size caches.
Point and continuous padding remain numeric; continuous padding retains its
