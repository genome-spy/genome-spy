...
test("conditional order isolates pass uniforms and preserves all-mode picking", async ({
    page,
}) => {
    ...
    const result = await page.evaluate(async () => {
        ...
        /** @param {Array<"all" | "matching" | "nonmatching">} passes */
        const read = async (passes) => {
            renderer.render({
                draws: passes.map((orderPass) => ({ mark, orderPass })),
        ...
        const empty = await read(["all"]);
        mark.selections.picked.set(41);
        const promoted = await read(["nonmatching", "matching"]);
        renderer.renderPicking({ draws: [{ mark, orderPass: "nonmatching" }] });
        const picked = await renderer.pick(64, 64);
        const reverse = await read(["matching", "nonmatching"]);
        mark.selections.picked.set(0);
        const cleared = await read(["all"]);
    ...
    expect(result.reverse).toEqual(result.empty);
