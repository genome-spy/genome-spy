...
describe("toolCatalog", () => {
    it("exposes the generated agent tools", () => {
        ...
        expect(tools[9].strict).toBe(true);
        expect(tools[10].strict).toBe(false);
    });
    ...
    it("builds Responses API function tool definitions", () => {
        ...
        expect(toolDefinitions[8].name).toBe(
            "getGroupedMetadataAttributeSummary"
        );
        expect(toolDefinitions[8].parameters.type).toBe("object");
        expect(toolDefinitions[8].parameters.required).toEqual(["attribute"]);
        expect(toolDefinitions[8].strict).toBe(true);
        expect(toolDefinitions[9].name).toBe("searchViewDatums");
        expect(toolDefinitions[9].parameters.type).toBe("object");
        expect(toolDefinitions[9].parameters.required).toEqual([
            "selector",
            "query",
            "field",
            "mode",
        ]);
        expect(toolDefinitions[9].parameters.properties.field).toEqual({
            type: "string",
            description:
                "Search field name. Use an empty string to search all configured fields.",
        });
        expect(toolDefinitions[9].parameters.properties.mode).toEqual({
            type: "string",
            enum: ["exact", "prefix"],
            description:
                "Search mode. `exact` matches the whole field value. `prefix` matches the beginning of the field value.",
        });
        expect(toolDefinitions[9].strict).toBe(true);
        expect(toolDefinitions[10].parameters.type).toBe("object");
        expect(toolDefinitions[10].strict).toBe(false);
    });
