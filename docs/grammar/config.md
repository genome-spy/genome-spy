...
Unlike Vega-Lite, configuration is **hierarchical** in GenomeSpy: `config` can
appear at multiple levels of the view hierarchy.

## Mental Model

Use this model when deciding which mechanism to use:

- `theme`: select a broad visual preset for the whole visualization
...
GenomeSpy supports built-in `theme` selection at the root specification.
...
!!! note
    Selecting a built-in theme in the root spec is supported, but **defining
    custom named themes inside the spec is not yet supported**.
...
- Use `theme` when you want a broad, recognizable look for the whole visualization.
...
4. Root `spec.theme`
5. Root `spec.config`
6. Ancestor view `config` (root to parent)
7. Import-site `config` (if any)
8. Imported root `config` (if any)
9. View-local `config`
...
- `theme` selects a full preset configuration for the root visualization.
