...
class Documentation:
    ...
    def markdown_url(self, source_path: Path) -> str:
        """Return a relative Markdown link from a documentation source file."""
        relative_path = Path(os.path.relpath(self.path, source_path.parent)).as_posix()
        return relative_path + (f"#{self.heading.anchor}" if self.heading else "")
...
def get_indexed_types(schema: dict) -> list[str]:
    """Return schema types that can be represented by Markdown headings."""
    return sorted(
        type_name
        for type_name in schema["definitions"]
        if TYPE_NAME_PATTERN.fullmatch(type_name)
    )


def render_types_page(
    type_names: list[str], documentation: dict[str, Documentation]
) -> str:
    ...
    for type_name in type_names:
        ...
        documented_type = documentation.get(type_name)
        if documented_type is None:
            lines.extend([f"SCHEMA {type_name}", ""])
        else:
            url = documented_type.markdown_url(TYPES_PATH)
            lines.extend([f"See the [{type_name} documentation]({url}).", ""])
    ...
    links.update(
        {
            ...
            if type_name not in documentation
        }
    )
...
def generate(
    ...
    indexed_types = get_indexed_types(schema)
    return (
        render_types_page(indexed_types, documentation),
